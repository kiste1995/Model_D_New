...
power_control_pub = rospy.Publisher(uvc_control_topic,PowerControlMsgs,queue_size=10)
...
def power_command_ack_CB(msg) :
    global power_control_ack_msg
    power_control_ack_msg = msg

def uvc_controller(msg) :
    pre_time = time()
    while time() - pre_time <= 3 :
        power_control_pub.publish(msg)
        try :
            power_control_command = rospy.wait_for_message("/power_control_command_ack",PowerControlMsgs,timeout=1)
        except :
            power_control_command = PowerControlMsgs()
        if msg.port == power_control_command.port and uvc_control_msg.state == msg.state :
            break

def purifier_controller(msg) :
    pre_time = time()
    while time() - pre_time <= 3 :
        purifier_pub.publish(msg)
        try :
            purifier_command_ack = rospy.wait_for_message("/purifier_control_command_ack",UInt16,timeout=1)
        except :
            purifier_command_ack = UInt16()
        if msg.data == purifier_command_ack.data :
            break
...
def module_controller(comm_list):
    ...
    for i in comm_list :
        ...
        if "led" in i :
            ...
            purifier_controller(purifier_command)
            ...
            uvc_controller(uvc_control_msg)
            print("")
...
def main() :
    ...
    rospy.Subscriber('/power_control_command_ack',PowerControlMsgs,power_command_ack_CB)
