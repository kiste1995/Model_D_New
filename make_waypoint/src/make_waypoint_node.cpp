...
#include <termios.h>
//#include <fstream>
#include <zetabank_msgs/SaveWaypoint.h>
...
void saveWPCallback(const zetabank_msgs::SaveWaypoint& str_msg);
...
void saveWPCallback(const zetabank_msgs::SaveWaypoint& str_msg)
{
    // std::string cmd_str = str_msg.data.c_str();
    std::string cmd_str = str_msg.command.c_str();
    std::string fname_str = str_msg.file_name.c_str();
    ROS_INFO_STREAM("Receive msgs(command) : " << cmd_str);
    ROS_INFO_STREAM("Receive msgs(save file name) : " << fname_str);

    geometry_msgs::Pose pos;

    if(cmd_str.compare("save") == 0)
    {
        ...
        sprintf(fname, "%s/%s.csv", path.c_str(), str_msg.file_name.c_str());
        ...
        sprintf(fname, "%s/%s.yaml", path.c_str(), str_msg.file_name.c_str());
...
void clickedPointCallback(const geometry_msgs::PointStamped& cp_msg)
{
    ...
    else if(pnum >= 1)
    {
        ...
        float oval = atan(diffx / fabs(diffy));
        if( diffx >= 0.0 && diffy >= 0.0)
            ang[pnum - 1] = oval;
        else if( diffx >= 0.0 && diffy < 0.0)
            ang[pnum - 1] = 180.0 - oval;
        else if( diffx < 0.0 && diffy >= 0.0)
            ang[pnum - 1] = 270.0 + oval;
        else if( diffx < 0.0 && diffy < 0.0)
            ang[pnum - 1] = 180.0 + oval;
        
        ROS_INFO("[1]num:%d (x:%f y:%f z:%f ang:%.2f)", pnum-1, x[pnum-1], y[pnum-1], z[pnum-1], ang[pnum-1]); 
        ROS_INFO("[2]num:%d (x:%f y:%f z:%f )", pnum, x[pnum], y[pnum], z[pnum]); 
    }
