...
def cancel_callback(data):
    global quit_fulcoverage_flag
    quit_fulcoverage_flag = data.data
    print("cancel_callback")

class FullCoverageAction(object):
    ...
    def __init__(self, name):
        ...
        self._as = actionlib.SimpleActionServer(self._action_name, ScheduleFullcoverageAction,execute_cb=self.execute_cb, auto_start=False)
        self._as.start()
    ...
    def execute_cb(self, goal):
        ...
        os.system("mplayer ~/voice/floor_start.mp3")
        success = fullcoverage_op()
        ...
        if success:
            os.system("mplayer ~/voice/floor_done.mp3")
            self._result.result = "end fulcoverage"
            rospy.loginfo('%s: Succeeded' % self._action_name)
            self._as.set_succeeded(self._result)
        else :
            os.system("mplayer ~/voice/floor_cancel.mp3")
            self._result.result = "cancel fulcoverage"
...
def fullcoverage_op():
    ...
    global p_file
    global p_file2
    fname = '/home/zetabank/catkin_ws/src/full_coverage/scripts/map/luna_map_full.pgm'
    ...
    #     coverage_list.pop()
    #     idx = idx + 1
    fullcoverage_op_state = execute_fulcoverage()
    #p_file.close()
    cv2.imshow('Original', original)
    ...
    return fullcoverage_op_state

if __name__ == '__main__':
    try:
        rospy.init_node('movebase_client_py')
        rospy.Subscriber("/map_metadata", MapMetaData ,mapcb)
        rospy.Subscriber("/floor_cleaning_cancel", Bool, cancel_callback)
    except rospy.ROSInterruptException:
        rospy.loginfo("Navigation test finished.")

    server = FullCoverageAction("fullCoverage_action")