...
scan_rectangle_F = []
scan_rectangle_L = []
scan_rectangle_F_right = []
scan_rectangle_F_center = []
scan_rectangle_F_left = []
lidar_warring = lidar_filter()
...
robot_width = 0.21
side_margin = 0.25
width_boundary = robot_width + side_margin
...
def recv_scan_rectangle_R(data):
    global scan_rectangle_R
    scan_rectangle_R = data.data

def recv_scan_rectangle_F(data):
    global scan_rectangle_F
    global scan_rectangle_F_right
    global scan_rectangle_F_left
    global scan_rectangle_F_center
    scan_rectangle_F = data.data
    scan_rectangle_F_right = data.data[0:29]
    scan_rectangle_F_center = data.data[30:59]
    scan_rectangle_F_left = data.data[60:89]

def recv_scan_rectangle_L(data):
    global scan_rectangle_L
    scan_rectangle_L = data.data

def recv_lidar_warring(msg):
    global lidar_warring
    lidar_warring = msg.warning
...
def mode_selector() :
    ...
    # if "front" in lidar_warring :
    #     return "close_corner" 

    for i in scan_rectangle_F_left :
        if i <= front_boundary :
...
def straight_tracer() :
    ...
    if abs(distance_err) > 0.001 :
        ...
        lidar_err = front_boundary - sum(scan_rectangle_F_right)/len(scan_rectangle_F_right)
        if lidar_err <= 0 :
            lidar_err = 0
        print(lidar_err)

        a_val = (distance_err + lidar_err*10)*2
        if a_val > max_a_val :
            a_val = max_a_val
...
def main():
    ...
    rospy.Subscriber(scan_rectangle_R_topic,Float64MultiArray,recv_scan_rectangle_R)
    rospy.Subscriber(scan_rectangle_F_topic,Float64MultiArray,recv_scan_rectangle_F)
    rospy.Subscriber(scan_rectangle_L_topic,Float64MultiArray,recv_scan_rectangle_L)

    lidar_warring_topic = "/lidar_warring"
    rospy.Subscriber(lidar_warring_topic,lidar_filter,recv_lidar_warring)
