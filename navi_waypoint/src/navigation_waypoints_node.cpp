...
class navigation_WayPoints
{
    public:
        ...
        double up_rear_sonar_dist;
        double up_left_sonar_dist;
        double up_right_sonar_dist;
        double down_front_sonar_dist;
...
void navigation_WayPoints::waypointNaviCtrl()
{
    ...
    while(ros::ok())
    {
        ...
        if(_state == START)
        {
            ...
            sonar_mutex.lock();
            double dfsd = down_front_sonar_dist;        
            sonar_mutex.unlock();   

            if(dfsd < th_frontdist)
            {            
                ROS_ERROR(">>> [Start] : The robot stops moving due to an obstacle in front of it.");
                ROS_INFO("df( %lf )", dfsd);
            } 
            else 
            {        
        ...
        else if(_state == ACTIVE)
        {
                ...
                (goal_state == actionlib::SimpleClientGoalState::PREEMPTED))
            {
                ...
                if(dfsd < th_frontdist)
                {
                    ...
                    ROS_ERROR(">>> [Active] :The robot stops moving due to an obstacle in front of it.");
        ...
        else if( _state == ABORTED)
        {
            if(abort_mode.compare("back") == 0)
            {
                ...
                if(bUseSonar == true)
                {
                    ...
                    double ursd = up_rear_sonar_dist;
                    double dfsd = down_front_sonar_dist;
                    sonar_mutex.unlock();               

                    if((drsd1 > th_reardist) && (drsd2 > th_reardist) && (ursd > th_reardist) && (dfsd < 50.0))
...
void navigation_WayPoints::SonarCallback(const std_msgs::Float32MultiArray& msg)
{
    ...
    // {
        ...
        down_left_sonar_dist[0] = sensor_sonar.data[0];
        down_left_sonar_dist[1] = sensor_sonar.data[1];

        down_front_sonar_dist = sensor_sonar.data[2];

        down_right_sonar_dist[0] = sensor_sonar.data[3];
        down_right_sonar_dist[1] = sensor_sonar.data[4];

        down_rear_sonar_dist[0] = sensor_sonar.data[5];
        down_rear_sonar_dist[1] = sensor_sonar.data[6];

        up_right_sonar_dist = sensor_sonar.data[7];
        up_rear_sonar_dist = sensor_sonar.data[8];
        up_left_sonar_dist = sensor_sonar.data[9];
