...
#include <zetabank_msgs/BatteryInformationMsgs.h>
...
zetabank_msgs::BatteryInformationMsgs battery;
...
ros::Publisher batvol_pub;
ros::Publisher batcur_pub;
ros::Publisher batsoc_pub;
...
void batteryCallback(const zetabank_msgs::BatteryInformationMsgs& msg);
...
void WheelControl(int* publish_rate)
{
    ...
    ros::Subscriber bat_subscriber = node_obj.subscribe("battery",100,batteryCallback);

    odom_pub = node_obj.advertise<nav_msgs::Odometry>("odom", 50);
    
    batvol_pub = node_obj.advertise<std_msgs::Float32>("battery_voltage", 10);
    batcur_pub = node_obj.advertise<std_msgs::Float32>("battery_current", 10);
    batsoc_pub = node_obj.advertise<std_msgs::UInt8>("battery_soc", 10);
    ...
    BYTE AlarmVal1 = 0, AlarmVal2 = 0;
    BYTE OLVal1 = 0, OLVal2 = 0;
	...
	while(ros::ok()){
        ...
        if((t - tTime[0]) >= (DRIVE_INFO_PUBLISH_PERIOD - 5)) {
            ...
            if(mdr_mc.b_recvOK)
            {
                ...
                left_motor_vel = INSIGN*(rpm2MPS(lm_vel));
                right_motor_vel = INSIGN*(rpm2MPS(rm_vel));
                left_motor_pos = INSIGN*pulse2MPS(lm_pos);
                right_motor_pos = INSIGN*pulse2MPS(rm_pos);
                ...
                // calculate odometry
                if(fabs(step_time/1000000000UL) > 0.1)
                {
                    time_now = ros::Time::now().toNSec(); 
                    prev_update_time = time_now;
                    step_time = 0.04*1000000000UL;
                }
                ...
                updateJointStates();
                joint_states.header.stamp = stamp_state;
            ...
            if(srcnt>25)
            {
                ...
                if(mdr_mc.isMDMCRun() != ON)
                {
                    ...
                    mdr_mc.getAlarm(&AlarmVal1, &AlarmVal2);                
                    if (AlarmVal1>0 || AlarmVal2>0)
                    {
                        ROS_ERROR("SET ALARM!!!!");
                        mdr_mc.getOverLoad(&OLVal1, &OLVal2);
                        if (OLVal1>0 || OLVal2>0)
                        {
                            ROS_WARN("OLVal1:%d OLVal2:%d", OLVal1, OLVal2);
                            mdr_mc.ResetAlarm();

                            ros::Rate rev_delay(500);
                            
                            mdr_mc.ResetAlarm();
                            ROS_WARN("Send Reset Alarm command...");
                        }
                    }
                }
            }
...
void batteryCallback(const zetabank_msgs::BatteryInformationMsgs& msg)
{
    ...
    vol.data = msg.voltage;
    std_msgs::Float32 cur;
    cur.data = msg.current;
    std_msgs::UInt8 soc;
    soc.data = msg.SOC;

    batvol_pub.publish(vol);
    batcur_pub.publish(cur);
    batsoc_pub.publish(soc);
 ...
 bool controlMotor(float * value)
{
    float wheel_velocity_cmd[2];
     ...
     wheel_velocity_cmd[LEFT]  = value[LINEAR] + ROTSIGN*(value[ANGULAR] * WHEEL_SEPARATION / 2.0f);
    wheel_velocity_cmd[RIGHT] = value[LINEAR] - ROTSIGN*(value[ANGULAR] * WHEEL_SEPARATION / 2.0f);
    ...
    wheel_velocity_cmd[LEFT] = LEFTSIGN*wheel_velocity_cmd[LEFT]*DISTORPM;
    wheel_velocity_cmd[RIGHT] = RIGHTSIGN*wheel_velocity_cmd[RIGHT]*DISTORPM;
...
bool calcOdometry(double diff_time)
{
    ...
    v = (LEFTSIGN*left_motor_vel + RIGHTSIGN*right_motor_vel) / 2.0;
    w = (RIGHTSIGN*right_motor_vel - (LEFTSIGN*left_motor_vel))/ WHEEL_SEPARATION;

    pre_left_motor_vel  = LEFTSIGN*left_motor_vel;
    pre_right_motor_vel = RIGHTSIGN*right_motor_vel;
