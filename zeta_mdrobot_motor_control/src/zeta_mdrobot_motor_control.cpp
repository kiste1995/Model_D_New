...
void mdrobot_motor_control::getAlarm(BYTE *resval1, BYTE *resval2)
{
	*resval1 = fgAlarm[MOTOR_LEFT];
	*resval2 = fgAlarm[MOTOR_RIGHT];
}

void mdrobot_motor_control::getOverLoad(BYTE *resval1, BYTE *resval2)
{
	*resval1 = fgOverLoad[MOTOR_LEFT];
	*resval2 = fgOverLoad[MOTOR_RIGHT];
}

BYTE mdrobot_motor_control::getCtrlStatus(void)
{
BYTE byTempDataSum = 0;
	int i;
...
#ifdef DEBUG_BASIC  
	ROS_INFO("Get Ctrl Status Send Data ==> ");
	for(int i = 0; i < 7; i++)
		ROS_INFO("(%d) : %d ", i, sendBuf[i]);
#endif		
	memset(sendBuf,0,MAX_PACKET_SIZE);
...
int mdrobot_motor_control::ParseReceiveData(void)
{
#ifdef _DEBUG_BASIC	
	ROS_INFO("recvPID:%d\n", recvBuffer[3]);
	// ROS_INFO("recv num::%d  recvPID:%d\n", recvNumber, recvBuffer[3]);
	...
	switch(recvBuffer[3])
	{
...
#ifdef _DEBUG_BASIC
			ROS_INFO("fgAlarm:(%d, %d)  fgOverLoad:(%d,%d)", fgAlarm[MOTOR_LEFT], fgAlarm[MOTOR_RIGHT],  fgOverLoad[MOTOR_LEFT], fgOverLoad[MOTOR_RIGHT]);
			// ROS_INFO("fgAlarm:(%d, %d)  fgOVerVolt:(%d,%d)  fgOverLoad:(%d,%d)", fgAlarm[MOTOR_LEFT], fgAlarm[MOTOR_RIGHT],  fgOverVolt[MOTOR_LEFT], fgOverVolt[MOTOR_RIGHT], fgOverLoad[MOTOR_LEFT], fgOverLoad[MOTOR_RIGHT]);

			// ROS_INFO("recvData => %d  %d", byStatus[MOTOR_LEFT], byStatus[MOTOR_RIGHT]);
			// int val1 = (byStatus[MOTOR_LEFT] & 0x10) >> 4;
			// int val2 = (byStatus[MOTOR_RIGHT] & 0x10) >> 4;
			// ROS_INFO("left motor overload : %d right motor overload : %d", val1, val2);
		...
		case PID_CTRL_STATUS:
			...
			ROS_INFO("fgAlarm:(%d, %d)  fgOVerVolt:(%d,%d)  fgOverLoad:(%d,%d)", fgAlarm[MOTOR_LEFT], fgAlarm[MOTOR_RIGHT],  
					fgOverVolt[MOTOR_LEFT], fgOverVolt[MOTOR_RIGHT], fgOverLoad[MOTOR_LEFT], fgOverLoad[MOTOR_RIGHT]);
